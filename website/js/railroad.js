...
export class Terminal extends FakeSVG {
	constructor(text, {href, title, cls}={}) {
      ...
      // This is not part of the original code. Added to support proper line wrapping.
      if (text.length < 15) {
          ...
          var num_lines = Math.ceil(text.length/15) + 1;
          this.width = 15 * 8 + 20;
          this.height = 0;
          this.up = (num_lines * 15) / 2;
          this.down = (num_lines * 15) / 2;

          var words = text.split(/\s+/).reverse(),
              line = [], lines = [], length = 0,
              word = words.pop();
          while (word) {
              if (length + word.length < 15) {
    ...
    new FakeSVG('rect', {x:x, y:y-this.down, width:this.width, height:this.up+this.down, rx:10, ry:10}).addTo(this);
    if (this.text instanceof String) {
        ...
        for (var i=0; i<this.text.length; i++) {
            if (i==0) {y += 4;} else {y += 13;}
            var text = new FakeSVG('text', {x:x+this.width/2, y:y}, this.text[i]);
            text.addTo(this);
        }
    }
		return this;
